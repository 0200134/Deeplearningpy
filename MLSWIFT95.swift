...
func generateImages(prompt: String, n: Int = 1, width: Int = 512, height: Int = 512, completion: @escaping (Result<[String], APIError>) -> Void) {
...
func saveImages(imageUrls: [String], directory: String = "generated_images", savePath: URL? = nil) {
    ...
    if let savePath = savePath {
        ...
        saveDirectory = documentsUrl.appendingPathComponent(directory)
    }
    ...
    do {
        ...
        for (index, imageUrl) in imageUrls.enumerated() {
            ...
            let imagePath = saveDirectory.appendingPathComponent("image_\(index + 1).png")
...
generateImages(prompt: prompt, n: 3, width: 1024, height: 1024) { result in
    switch result {
