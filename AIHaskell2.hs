...
-- Main function to initialize and train the network with error handling
main :: IO ()
main = do
  ...
  result <- (do
    ...
    print trainedBias
    return $ Just (trainedWeights, trainedBias)
    ) `catch` handler
  case result of
    Just (w, b) -> putStrLn "Training completed successfully."
    Nothing -> putStrLn "An error occurred during training."

-- Exception handler
handler :: IOException -> IO (Maybe (Matrix Double, Vector Double))
handler ex = do
